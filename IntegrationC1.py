...
def integrate(district,rfdistrict,pressdistrict,tempdistrict,rfdistrictl,pressdistrictl,tempdistrictl,districtdroughtyn):
	...
	for year in range(16):
		for month in range(12):
			...
			rf=rfdistrictl.loc[month,years[year]]
			pressaverage=avglabel(pressdistrict[pressdistrict.columns[month].encode('ascii')].mean(),1000,1017,6)
			press=pressdistrictl.loc[month,years[year]]
			tempaverage=avglabel(tempdistrict[tempdistrict.columns[month].encode('ascii')].mean(),18,39,5)
			temp=tempdistrictl.loc[month,years[year]]
			drought=districtdroughtyn.loc[month,years[year]]
			Classifier1Data.loc[count]=[district,years[year],months[month],rf,rfaverage,press,pressaverage,temp,tempaverage,drought]
			count+=1
...
writer = pd.ExcelWriter('Classifier1Data.xlsx', engine='xlsxwriter')
