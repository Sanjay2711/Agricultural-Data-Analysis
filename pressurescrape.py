...
while(i<=2016 and j<=12):
	print(i,j)
	url="https://www.timeanddate.com/weather/india/latur/historic?month="+str(j)+"&year="+str(i)
	r=requests.get(url)
	soup=BeautifulSoup(r.text,"lxml")
	table=soup.find('table',{'class':'zebra tb-wt fw tb-hover'})
	td=float(table.find('tbody').find('tr',{'class':'sep-t'}).find('th',text='Average').findNext('td').findNext('td').findNext('td').text.split()[0])
	print(td)
	df[str(i)][str(j)]=td
	j+=1
	if(j>12 and i<2016):
